...
var formatInputAuthorization =function (auth, fieldNumber) {
    ...
    // Data
    w.bytes(value);
    console.log("Serialized authorization. value length ", value.length);
    return w.finish();
};
...
var formatInputProposal = function (proposal, fieldNumber) {
    ...
    console.log('proposal txn - ', proposal.TxnData);
    var types = proposal.TxnData.MethodAbi.Params.map(function (i) {
        return i.Type;
    });
    //var c = new coder();
    var coder = require('./coder');
    var raw_txn = proto.getTransaction(proposal.TxnData.From, proposal.TxnData.To, proposal.TxnData.MethodName, coder.encodeParams(types, proposal.TxnData.Params));
    console.log('raw_txn - ', raw_txn);
    var txn_data = proto.Transaction.encode(raw_txn).finish();
    console.log('txn data length-', txn_data.length);
    var p = proto.getProposal(proposal.MultiSigAccount, proposal.Name, txn_data, proposal.ExpiredTime, proposal.Proposer);
    console.log('proposal- ', proposal);
    var value = proto.Proposal.encode(p).finish();
    var w = new protobuf.BufferWriter();
    // Tag
    w.uint32(fieldNumber << 3 | 2);
    // Data
    w.bytes(value);
    console.log("Serialized proposal. value length ", value.length);
    return w.finish();
};
