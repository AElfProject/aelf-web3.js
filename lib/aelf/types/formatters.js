...
var formatInputProposal = function (proposal, fieldNumber) {
    ...
    console.log('txn from - ', txn_data.From);
    var t = new protobuf.BufferWriter();
    // Tag
    t.uint32(fieldNumber << 3 | 2);
    // Data
    t.bytes(txn_data);
    var data = t.finish();
    console.log('txn data length - ',  data.length);

    var p = proto.getProposal(proposal.MultiSigAccount, proposal.Name, data, proposal.ExpiredTime, proposal.Proposer);
