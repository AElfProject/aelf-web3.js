 ...
 */
var ContractMethod = function (chain, methodAbi, address) {
    this._chain = chain;
...
ContractMethod.prototype.toPayload = function (args) {
    var rawtx = proto.getTransaction(config.defaultAccount, this._address, this._name, coder.encodeParams(this._paramTypes, args))
    var incr = this._chain.getIncrement(config.defaultAccount).result.increment;
