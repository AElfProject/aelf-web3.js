...
ContractMethod.prototype.toPayload = function (args) {
    ...
    let block_height = JSON.parse(this._chain.getBlockHeight({sync: true}), 10);
    let block_info = this._chain.getBlockByHeight(block_height, false, {sync: true});
...
ContractMethod.prototype.unpackOutput = function (output) {
    ...
    let buffer = Buffer.from(output, 'hex');
    let decoded = this._outputType.decode(buffer);
...
ContractMethod.prototype.sendTransaction = function () {
    ...
    if (isSync) {
        ...
        return this._chain.sendTransaction(payload, {sync: true});
    }
...
ContractMethod.prototype.callReadOnly = function () {
    ...
    if (isSync) {
        let payload = this.toPayload(args);
        let returnBytes = this._chain.callReadOnly(payload, {sync: true});
